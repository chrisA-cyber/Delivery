...
export function AuthPanel({ enabledProviders = [], emailReady = false, passwordSignIn = false }: {
  ...
  return <div className="panel-solid w-full max-w-lg p-6 sm:p-9">
    ...
    <Link href={next.startsWith("/a/") || next.startsWith("/switch") || next.startsWith("/say-it-back") || next.startsWith("/play") || next.startsWith("/challenge/") || next.startsWith("/rounds") ? next : "/play"} className="button-ghost mt-1 w-full">Keep playing as guest</Link>
