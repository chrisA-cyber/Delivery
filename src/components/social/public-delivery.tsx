...
export function PublicDelivery({ deliveryId }: { deliveryId: string }) {
  ...
  return <div className="mx-auto max-w-4xl">
    ...
    <section className="panel-solid relative overflow-hidden p-5 sm:p-8">
      ...
      <div className="relative mt-8 rounded-xl bg-[#f4f0e7] p-5 text-[#171715] sm:p-7"><p className="mono-label text-black/65">The line</p><p className="mt-3 text-2xl font-black leading-8">“{delivery.promptText}”</p>{delivery.energy && <p className="mt-4 flex items-start gap-2 text-sm font-bold leading-6 text-black/65"><Sparkles className="mt-1 size-3.5 shrink-0 text-hot" /> {delivery.energy}</p>}</div>
      {delivery.audioUrl ? <div className="relative mt-5 rounded-2xl border border-electric/20 bg-electric/[0.08] p-4"><p className="mb-3 flex items-center gap-2 text-xs font-black text-electric"><Headphones className="size-4" /> Hear the evidence</p><audio controls preload="metadata" className="w-full" src={delivery.audioUrl}>Your browser cannot play this audio.</audio></div> : <p className="relative mt-5 text-center text-xs font-bold text-white/60">Audio is unavailable, but the receipts remain.</p>}
