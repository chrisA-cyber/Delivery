...
export function PublicDelivery({ deliveryId }: { deliveryId: string }) {
  ...
  if (error) return <div className="panel-solid mx-auto max-w-xl p-7 text-center"><p className="mono-label text-hot">Tape unavailable</p><h1 className="mt-4 text-3xl font-black">Private, removed, or lost to the timeline.</h1><p className="mt-3 text-sm leading-6 text-white/45">{error}</p><Link href="/play" className="button-primary mt-7">Make your own <ArrowRight className="size-4" /></Link></div>;
  if (!delivery) return <div className="panel mx-auto grid min-h-96 max-w-xl place-content-center text-center"><LoaderCircle className="mx-auto size-7 animate-spin text-acid" /><p className="mono-label mt-4 text-white/40">Rewinding the tape</p></div>;

  return <div className="mx-auto max-w-4xl">
    <div className="mb-5 flex items-center justify-between"><span className="mono-label text-acid">Public delivery</span><span className="mono-label text-white/30">{formatDate(delivery.createdAt)}</span></div>
    <section className="panel-solid relative overflow-hidden p-5 sm:p-8">
      ...
      <div className="relative mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-5"><p className="mono-label text-acid">The line</p><p className="mt-3 text-2xl font-black leading-8">“{delivery.promptText}”</p>{delivery.energy && <p className="mt-4 flex items-start gap-2 text-sm font-bold leading-6 text-white/50"><Sparkles className="mt-1 size-3.5 shrink-0 text-hot" /> {delivery.energy}</p>}</div>
      {delivery.audioUrl ? <div className="relative mt-5 rounded-2xl border border-electric/20 bg-electric/[0.08] p-4"><p className="mb-3 flex items-center gap-2 text-xs font-black text-electric"><Headphones className="size-4" /> Hear the evidence</p><audio controls preload="metadata" className="w-full" src={delivery.audioUrl}>Your browser cannot play this audio.</audio></div> : <p className="relative mt-5 text-center text-xs font-bold text-white/30">Audio is unavailable, but the receipts remain.</p>}
      <div className="relative mt-7 grid gap-3 sm:grid-cols-4">{scoreLabels.map(([key, label]) => <div key={key} className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><p className="display-type text-3xl text-acid">{delivery.scores[key]}</p><p className="mono-label mt-2 text-white/35">{label}</p></div>)}</div>
    </section>
