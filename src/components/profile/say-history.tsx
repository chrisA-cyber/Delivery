...
export function SayHistory() {
  ...
  return <section className="mt-10" aria-labelledby="dub-history-title">
    ...
    {attempts.length ? <div className="grid gap-4 md:grid-cols-2">{attempts.map((attempt) => <article key={attempt.id} className="panel overflow-hidden">
      ...
      <div className="p-5"><div className="flex items-start justify-between gap-4"><div><p className="mono-label text-electric">Say It Back</p><h3 className="mt-2 text-xl font-bold">{attempt.clip.title}</h3><p className="mt-2 text-xs text-white/60">{formatDate(attempt.createdAt)} · {attempt.clip.roles.find((role) => role.id === attempt.roleId)?.name}</p></div><div className="shrink-0 text-right"><p className="display-type text-4xl text-acid">{attempt.score?.overall ?? "—"}</p><p className="mt-1 text-[10px] font-bold text-white/55">{attempt.score ? attempt.score.timing == null || attempt.score.rhythm == null ? "Words only" : "Dialogue match" : "Not scored yet"}</p></div></div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2"><Link href={`/say-it-back?attempt=${attempt.id}`} className="button-secondary text-xs"><Play className="size-4" /> Replay, compare & video</Link>
