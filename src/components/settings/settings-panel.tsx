...
export function SettingsPanel() {
  const { muted, reducedMotion, publicDefault, notifications, authenticated, accountEmail, tier, profile, refreshAccount, updatePreferences, clearLocalData, signOut } = useApp();
  ...
  const navigation = [{ label: "Playback", icon: Volume2 }, { label: "Privacy", icon: ShieldCheck }, { label: "Notifications", icon: Bell }, { label: "Account", icon: Eye }];

  return (
    <div className="grid gap-5 lg:grid-cols-[240px_1fr]">
      <nav className="panel h-fit p-2">{navigation.map(({ label, icon: Icon }, index) => <a key={label} href={`#${label.toLowerCase()}`} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-black ${index === 0 ? "bg-white/10 text-white" : "text-white/45 hover:text-white"}`}><Icon className="size-4" /> {label}</a>)}</nav>
      <div className="grid gap-5">
        <Section id="playback" title="Playback & motion" description="Tune the game to your device and your nervous system.">
          <Toggle title="Playback audio" description="Mute take playback on this device." icon={Volume2} checked={!muted} onChange={(value) => updatePreferences({ muted: !value })} />
        ...
        <Section id="privacy" title="Privacy" description="Your voice belongs to you. New recordings are private until you choose otherwise.">
          <Toggle title="Publish by default" description="Run the safety check, then allow new judged deliveries to appear publicly." icon={Eye} checked={publicDefault} onChange={(value) => updatePreferences({ publicDefault: value })} />
          ...
          <div className="rounded-xl border border-acid/15 bg-acid/[0.06] p-4 text-xs font-bold leading-5 text-white/55"><ShieldCheck className="mb-2 size-4 text-acid" /> Guest takes are never uploaded to a public profile. Signed-in takes remain private unless this is enabled or you publish them later from your profile.</div>
          {authenticated && <div className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-4"><div className="flex items-center gap-2"><Ban className="size-4 text-white/40" /><p className="text-sm font-black">Blocked performers</p></div>{blocksLoading ? <p className="mt-3 text-xs text-white/35">Loading your list…</p> : blockedProfiles.length ? <div className="mt-3 grid gap-2">{blockedProfiles.map((blocked) => <div key={blocked.handle} className="flex items-center justify-between gap-3 rounded-xl bg-black/20 px-3 py-2"><div><p className="text-xs font-black">{blocked.displayName}</p><p className="text-[10px] font-bold text-white/30">@{blocked.handle}</p></div><button onClick={() => void unblock(blocked.handle)} className="button-ghost min-h-8 px-3 text-[10px]">Unblock</button></div>)}</div> : <p className="mt-3 text-xs leading-5 text-white/35">Nobody is blocked. Use Block on a performer&apos;s public profile to hide each other&apos;s content and remove follows.</p>}</div>}
          {profileError && <p role="alert" className="text-xs font-bold text-orange-200">{profileError}</p>}
        </Section>

        <Section id="notifications" title="Reminder preference" description="Saved locally now; push delivery is not enabled until the notification service is configured.">
          <Toggle title="Opt into Daily Drop reminders" description="Remember this preference on this device without requesting browser notification permission yet." icon={Bell} checked={notifications} onChange={(value) => updatePreferences({ notifications: value })} />
        </Section>
...
function Toggle({ title, description, icon: Icon, checked, onChange }: { title: string; description: string; icon: typeof Bell; checked: boolean; onChange: (value: boolean) => void }) {
  return <button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)} className="flex items-center justify-between gap-4 rounded-xl border border-white/[0.08] bg-white/[0.025] p-4 text-left"><div className="flex items-start gap-3"><Icon className="mt-0.5 size-4 text-white/40" /><div><p className="text-sm font-black">{title}</p><p className="mt-1 text-xs leading-5 text-white/55">{description}</p></div></div><span className={`relative h-7 w-12 shrink-0 rounded-full transition ${checked ? "bg-acid" : "bg-white/10"}`}><span className={`absolute top-1 size-5 rounded-full bg-black transition ${checked ? "left-6" : "left-1 bg-white/60"}`} /></span></button>;
}
