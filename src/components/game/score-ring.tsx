  ...
  return (
    <div className="relative" role="img" aria-label={`${label}: ${score} out of 100`} style={{ width: dimension, height: dimension }}>
      <svg width={dimension} height={dimension} className="-rotate-90" aria-hidden="true">
        <circle cx={dimension / 2} cy={dimension / 2} r={radius} fill="rgba(202,255,51,.05)" stroke="rgba(255,255,255,.08)" strokeWidth={stroke} />
        <motion.circle
          cx={dimension / 2}
          cy={dimension / 2}
          r={radius}
          ...
          style={{ filter: "drop-shadow(0 0 10px rgba(202,255,51,.5))" }}
        />
      </svg>
      <div className="absolute inset-0 grid place-content-center text-center">
        ...
        <span className="mono-label mt-1 text-[9px] text-white/35">{label}</span>
      </div>
    </div>
  );
}
