...
export function SayItBackExperience({ initialClipId, initialRoleId, initialAttemptId, challengeToken, initialClaimId, roundContext }: { initialClipId?: string; initialRoleId?: string; initialAttemptId?: string; challengeToken?: string; initialClaimId?: string; roundContext?: SayRoundContext }) {
  ...
  }, [contentRating, initialAttemptId, initialClaimId, challengeToken, initialClipId, initialRoleId, selectClip, authenticated, refreshAccount, roundContext]);
  ...
  useEffect(() => {
    ...
    window.history.replaceState(window.history.state, "", `${roundContext ? `/rounds/${encodeURIComponent(roundContext.token)}/record` : "/say-it-back"}${destination.size ? `?${destination}` : ""}`);
  }, [authReady, catalogLoading, loadError, attempt, clip, role, challenge, roundContext]);
  ...
  const signInAndKeepTake = useCallback(async () => {
    ...
    try {
      ...
      if (challenge) destination.set("challenge", challenge.token);
      const path = roundContext ? `/rounds/${encodeURIComponent(roundContext.token)}/record` : "/say-it-back";
  ...
  }, [attempt, audioBlob, uploadTake, captureBusy, clip, role, challenge, router, roundContext]);
  ...
  if (!clip || !role) return <main className="min-h-screen px-4 pb-28 pt-28 sm:px-8 sm:pt-32"><div className="mx-auto max-w-[1184px]">
    ...
    <p className="mb-6 text-xs leading-6 text-white/60">{tier === "pro" ? "Record, replay, and match your scenes with your Pro allowance." : "5 scored plays per day, shared with Classic. Recording and replaying are free. Resets at midnight UTC."}{!authenticated && " Play solo, or create and join friend rounds as a guest. Sign in to keep personal history and retain group access across devices."}</p>
  ...
  return <main className="min-h-screen px-4 pb-16 pt-[96px] sm:px-8 sm:pt-28"><div className="mx-auto max-w-[1184px]">
    ...
    {!selectedAllowed ? <section className="panel p-7"><h1 className="text-2xl font-bold">This scene is outside your content setting.</h1><p className="my-4 text-sm text-white/65">It is classified {CONTENT_LABELS[clip.rating]}. Choose the appropriate setting to open the scene and its recordings.</p><ContentControl value={contentRating} onChange={(rating) => updatePreferences({ contentRating: rating })} /></section> : <>
      ...
      {!roundContext && <div className="mb-5"><Link className="button-secondary inline-flex" href={`/rounds?mode=say-it-back&clip=${encodeURIComponent(clip.id)}&role=${encodeURIComponent(role.id)}`}><Users className="size-4" />Start a group round with this scene</Link><p className="mt-2 text-xs leading-5 text-white/55">One invitation link. Up to 12 friends. Reveal everyone’s performances together.</p></div>}
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.65fr)_minmax(285px,1fr)]">
        <div ref={sceneRef} className="min-w-0 scroll-mt-24 space-y-5">
          {!takeUrl && !captureBusy && <div className="rounded-xl border border-hot/25 bg-hot/[0.07] px-4 py-3 text-sm leading-6"><p className="font-bold">You’re playing {role.name}.</p><p className="mt-1 text-white/65">{recordingMode === "lines" ? "Listen to the scene, then record one line at a time. Each line waits for you. You can redo a line without starting over." : "Listen to the scene, then record all your dialogue in one take. Start when you’re ready; you’ll get a 3-second countdown."}</p></div>}
          ...
          <section className="rounded-2xl border border-white/15 bg-[#20201d] p-4 sm:p-5" aria-label="Recording controls">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div><p className="mono-label text-white/50">Your recording · take {takeNumber}</p><h2 className="mt-1 text-lg font-bold">{captureBusy ? activeWindow ? `Recording line ${lineWindows.findIndex((item) => item.cue.id === activeWindow.cue.id) + 1}` : "Recording the scene" : editing ? recordingMode === "lines" ? "Take your time. One line at a time." : "Ready when you are. Record the full scene." : "Ready for another go?"}</h2></div>
            ...
            {(editing || captureBusy) && <>
              ...
              {recordingMode === "lines" && <div className="mb-4 mt-4 space-y-2" aria-label="Record individual lines">{lineWindows.map((window, index) => {
                ...
                return <div key={window.cue.id} className={cn("rounded-xl border p-3", selected ? "border-acid/60 bg-acid/5" : "border-white/10 bg-black/10")}>
                  <button type="button" disabled={captureBusy} onClick={() => setSelectedLine(index)} className="w-full text-left"><span className="flex items-center justify-between gap-2 text-[10px] font-bold uppercase tracking-wide text-white/50"><span>Line {index + 1} · {window.start.toFixed(2)}–{window.end.toFixed(2)}s</span>{kept && <span className="flex items-center gap-1 text-acid"><Check className="size-3" />Kept</span>}</span><span className="mt-1 block text-sm font-bold leading-6">{window.cue.text}</span><span className="mt-1 block text-[10px] font-normal text-white/50">Dialogue {window.cue.start.toFixed(2)}–{window.cue.end.toFixed(2)}s · the recording range includes its pauses</span></button>
