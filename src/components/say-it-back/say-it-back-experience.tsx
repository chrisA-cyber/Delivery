...
function ScorePanel({ score, previousBest }: { score: SayScore; previousBest?: number | null }) {
  ...
  return <section className="overflow-hidden rounded-2xl bg-paper text-ink" aria-label="Your matching result">
    <div className="grid gap-5 p-6 sm:grid-cols-[150px_1fr] sm:items-center sm:p-7">
      ...
      <div><h2 className="text-2xl font-bold leading-tight tracking-tight sm:text-3xl">{title}</h2><p className="mt-2 text-sm leading-6 text-ink/70">{score.coachNote}</p>{comparable && <p className="mt-3 flex items-center gap-2 text-xs font-bold"><Trophy className="size-4" />{score.overall > previousBest! ? `New best · +${Math.round(score.overall - previousBest!)} points` : `Previous best: ${Math.round(previousBest!)}`}</p>}</div>
    </div>
...
export function SayItBackExperience({ initialClipId, initialRoleId, initialAttemptId, challengeToken, initialClaimId, roundContext, publicAssignment }: { initialClipId?: string; initialRoleId?: string; initialAttemptId?: string; challengeToken?: string; initialClaimId?: string; roundContext?: SayRoundContext; publicAssignment?: { code: string; assignment: Extract<GroupAssignment, { mode: "say-it-back" }> } }) {
  ...
  useEffect(() => {
    if (!sharing) return;
    sharePanelRef.current?.scrollIntoView({ block: "center", behavior: window.matchMedia?.("(prefers-reduced-motion: reduce)").matches ? "instant" : "smooth" });
  ...
  useEffect(() => {
    ...
    // Align after the compact capture layout has settled, including when the
    // record button was below the scene or a permission prompt took focus.
    const frame = requestAnimationFrame(() => sceneRef.current?.scrollIntoView({ block: "start", behavior: "instant" }));
  ...
  if (creatingScene && !clip && !roundContext && !publicAssignment) return <main data-mode="say-it-back" className="min-h-screen px-4 pb-28 pt-24 sm:px-8 sm:pt-28"><CustomSceneCreator key={resumingImport?.id ?? "new"} initialImport={resumingImport} initialRating={contentRating} authenticated={authenticated} onCreated={openCustomScene} onClose={() => { setCreatingScene(false); setResumingImport(undefined); setCatalogTab("mine"); }} /></main>;

  if (!clip || !role) return <main data-mode="say-it-back" className="min-h-screen px-4 pb-28 pt-24 sm:px-8 sm:pt-28"><div className="mx-auto max-w-[1184px]">
    <header className="mb-5 grid gap-4 border-b border-white/15 pb-5 lg:grid-cols-[1.15fr_1fr] lg:items-end">
      <div><p className="mono-label mb-3 flex items-center gap-2 text-electric"><Clapperboard className="size-4" />Dub the scene</p><h1 className="display-type text-[clamp(3.2rem,7vw,5rem)] leading-[.86]">SAY IT <span className="text-electric">BACK.</span></h1></div>
      <div><p className="max-w-lg text-lg leading-7 text-white/75">Watch a scene. Record the lines. Play your dub.</p><div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 text-xs font-bold text-white/55"><span className="flex items-center gap-1.5"><Mic className="size-4 text-acid" />Your voice. Camera optional.</span><span className="flex items-center gap-1.5"><Users className="size-4 text-electric" />Solo or against a friend.</span></div></div>
    </header>
    <div className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-xl font-bold tracking-tight">Choose your scene</h2><p className="mt-1 text-xs leading-5 text-white/55">Record line by line or perform the full scene.</p></div><button type="button" className="button-secondary" onClick={() => { initialLoadDone.current = true; setResumingImport(undefined); setCreatingScene(true); }}><Plus className="size-4" />Make your own</button></div>
  ...
  return <main data-mode="say-it-back" data-stage={captureBusy ? "recording" : editing ? "prompt" : currentScore ? "result" : "review"} className="say-workspace-page min-h-screen px-3 pb-10 pt-[84px] sm:px-6"><div className="mx-auto max-w-[1184px]">
    <div className="mb-2 flex flex-wrap items-center justify-between gap-3"><button type="button" className="button-ghost -ml-3 px-3" disabled={!canTryAgain} onClick={roundContext ? () => router.push(roundContext.returnPath) : publicAssignment ? () => router.push("/say-it-back") : backToScenes}><ArrowLeft className="size-4" />{roundContext ? "Back to round" : "All scenes"}</button><div className="flex items-center gap-3"><Link href="/switch" className="text-xs text-white/65 hover:text-white">Switch</Link><span className="h-4 w-px bg-white/20" /><span className="mono-label text-electric">Say It Back</span></div></div>
    ...
    {!selectedAllowed ? <section className="panel p-7"><h1 className="text-2xl font-bold">This scene is outside your content setting.</h1><p className="my-4 text-sm text-white/65">It is classified {CONTENT_LABELS[clip.rating]}. Choose the appropriate setting to open the scene and its recordings.</p><ContentControl value={contentRating} onChange={(rating) => updatePreferences({ contentRating: rating })} /></section> : <>
      <div ref={sceneRef} className="say-workspace scroll-mt-[84px]">
        <header className="say-workspace-heading mb-3 flex min-w-0 items-center justify-between gap-3">
          <div className="min-w-0"><p className="mono-label mb-1 text-electric">Say It Back · {durationLabel(clip.duration)}</p><h1 className="truncate text-lg font-bold tracking-tight sm:text-2xl" title={clip.title}>{clip.title}</h1></div>
        ...
        <div className="say-recording-grid">
          ...
          <div className="say-appearance-setup"><CameraControls camera={recorder.camera} disabled={captureBusy || Boolean(busy)} /></div>
          ...
          <section className="say-recorder min-w-0 rounded-2xl border border-white/15 bg-surface p-3 sm:p-4" aria-label="Recording controls">
            ...
            {(editing || captureBusy) && <>
              ...
              {!captureBusy && <div className="mt-2 flex min-h-10 items-center justify-between gap-2 text-xs">
              ...
              {!captureBusy && takeUrl && focusedIndex < lineWindows.length - 1 && <button type="button" className="button-ghost min-h-10 w-full text-xs" onClick={reviewDub}><Play className="size-3.5" />{fullLineTake ? "Review dub" : "Preview recorded lines"}</button>}
            ...
            {takeUrl && !captureBusy && !editing && <div className="mt-4 border-t border-white/10 pt-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" className="button-secondary min-h-11 px-3 text-xs" onClick={reviewDub}><Play className="size-4" />Replay dub</button><button type="button" aria-label="Record another take" className="button-secondary min-h-11 px-3 text-xs" onClick={retake} disabled={!canTryAgain}><RotateCcw className="size-4" />Another take</button></div>
      ...
      <div className="mt-4 grid items-start gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        ...
        <aside className="min-w-0 space-y-4">
          ...
          <p className="px-1 text-xs leading-5 text-white/55">{tier === "pro" ? "Scored plays use your Pro allowance." : remainingMatches == null ? "5 scored plays daily across modes. Resets midnight UTC." : `${remainingMatches} scored ${remainingMatches === 1 ? "play" : "plays"} left today across modes. Resets midnight UTC.`} Recording, replay, and saving do not use a scored play.</p>
