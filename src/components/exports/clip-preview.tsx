...
import { audioLevelAt, avatarFrame, cameraFrame, fullFrameCamera, compositionSvg, sceneFrame, type ClipEditSettings, type CompositionScene } from "@/lib/video-composition";
...
export function ClipPreview({ source, settings, onChange, disabled = false }: {
  ...
  const cropping = settings.performer === "camera" && (fullCamera || cropMode);
  const avatarBounds = settings.performer === "camera" ? cameraFrame(settings) : avatarFrame(settings);
  ...
  return <div className="flex h-full min-h-0 w-full flex-col items-center gap-2">
    ...
    <div className="flex min-h-0 w-full flex-1 items-center justify-center">
      <div ref={frame} className="relative isolate aspect-[9/16] h-full max-h-full max-w-full overflow-hidden rounded-xl bg-black shadow-2xl" style={{ width: "auto" }} aria-label="Editable clip preview">
        ...
        {!fullCamera && cameraLayer}
        {settings.avatarVisible && <button type="button" aria-label={settings.performer === "camera" ? cropping ? "Crop camera. Drag, or use arrow keys." : "Move camera. Drag, or use arrow keys." : "Move avatar. Drag, or use arrow keys."} disabled={disabled} className={`absolute touch-none rounded-full border-2 outline-none focus-visible:border-electric ${dragging ? "cursor-grabbing border-electric" : "cursor-grab border-transparent hover:border-white/60"}`} style={{ left: `${centerX * 100}%`, top: `${centerY * 100}%`, width: `${avatarBounds.width / 1080 * 100}%`, height: `${avatarBounds.height / 1920 * 100}%`, borderRadius: fullCamera ? 0 : undefined, transform: "translate(-50%, -50%)" }}
          onPointerDown={(event) => { if (event.button !== 0) return; event.preventDefault(); event.currentTarget.setPointerCapture?.(event.pointerId); drag.current = { id: event.pointerId, x: event.clientX, y: event.clientY, centerX: cropping ? settings.cameraCropX : centerX, centerY: cropping ? settings.cameraCropY : centerY }; setDragging(true); }}
          onPointerMove={(event) => { const current = drag.current; const bounds = frame.current?.getBoundingClientRect(); if (!current || current.id !== event.pointerId || !bounds?.width || !bounds.height) return; move(current.centerX + (event.clientX - current.x) / bounds.width * (cropping ? cameraMirrored ? 2 : -2 : 1), current.centerY + (event.clientY - current.y) / bounds.height * (cropping ? -2 : 1)); }}
          onPointerUp={() => { drag.current = null; setDragging(false); }} onPointerCancel={() => { drag.current = null; setDragging(false); }}
          onKeyDown={(event) => { const step = event.shiftKey ? 0.05 : 0.01; const directions: Record<string, [number, number]> = { ArrowLeft: [-step, 0], ArrowRight: [step, 0], ArrowUp: [0, -step], ArrowDown: [0, step] }; const delta = directions[event.key]; if (delta) { event.preventDefault(); move((cropping ? settings.cameraCropX : centerX) + delta[0] * (cropping && !cameraMirrored ? -1 : 1), (cropping ? settings.cameraCropY : centerY) + delta[1] * (cropping ? -1 : 1)); } }}>
          {dragging && <Move className="absolute left-1/2 top-1/2 size-5 -translate-x-1/2 -translate-y-1/2 rounded bg-black/65 text-white" />}
        </button>}
        {settings.avatarVisible && !fullCamera && <button type="button" disabled={disabled} aria-label="Resize performer. Drag, or use arrow keys." className="absolute z-10 size-7 touch-none rounded-full border-2 border-white/70 bg-ink/80 focus-visible:outline focus-visible:outline-electric" style={{ left: `${(avatarBounds.x + avatarBounds.width) / 1080 * 100}%`, top: `${(avatarBounds.y + avatarBounds.height) / 1920 * 100}%`, transform: "translate(-50%,-50%)" }}
