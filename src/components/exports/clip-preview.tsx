...
export function ClipPreview({ source, settings, onChange, disabled = false }: {
  ...
  const move = (x: number, y: number) => onChange({ avatarX: bound(x, settings.avatarSize / 2, 1 - settings.avatarSize / 2), avatarY: bound(y, settings.avatarSize * 1080 / 1920 / 2, 1 - settings.avatarSize * 1080 / 1920 / 2) });
    ...
    <div className="flex min-h-0 w-full flex-1 items-center justify-center">
      <div ref={frame} className="relative isolate aspect-[9/16] h-full max-h-full max-w-full overflow-hidden rounded-xl bg-black shadow-2xl" style={{ width: "auto" }} aria-label="Editable clip preview">
        ...
        {settings.avatarVisible && <button type="button" aria-label="Move avatar. Drag, or use arrow keys." disabled={disabled} className={`absolute touch-none rounded-full border-2 outline-none focus-visible:border-electric ${dragging ? "cursor-grabbing border-electric" : "cursor-grab border-transparent hover:border-white/60"}`} style={{ left: `${centerX * 100}%`, top: `${centerY * 100}%`, width: `${settings.avatarSize * 100}%`, aspectRatio: "1", transform: "translate(-50%, -50%)" }}
          ...
          onPointerMove={(event) => { const current = drag.current; const bounds = frame.current?.getBoundingClientRect(); if (!current || current.id !== event.pointerId || !bounds?.width || !bounds.height) return; move(current.centerX + (event.clientX - current.x) / bounds.width, current.centerY + (event.clientY - current.y) / bounds.height); }}
          onPointerUp={() => { drag.current = null; setDragging(false); }} onPointerCancel={() => { drag.current = null; setDragging(false); }}
          onKeyDown={(event) => { const step = event.shiftKey ? 0.05 : 0.01; const directions: Record<string, [number, number]> = { ArrowLeft: [-step, 0], ArrowRight: [step, 0], ArrowUp: [0, -step], ArrowDown: [0, step] }; const delta = directions[event.key]; if (delta) { event.preventDefault(); move(centerX + delta[0], centerY + delta[1]); } }}>
