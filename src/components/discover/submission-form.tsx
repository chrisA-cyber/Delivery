...
export function SubmissionForm() {
  ...
  return <div className="grid gap-5 lg:grid-cols-[1fr_360px] lg:items-start">
    <form onSubmit={submit} className="panel-solid p-5 sm:p-8">
      ...
      <textarea id="submission-line" required minLength={4} maxLength={180} value={text} onChange={(event) => setText(event.target.value)} placeholder="The meeting could have been an email, but the lore demanded witnesses." className="mt-3 min-h-36 w-full resize-y rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-lg font-black leading-7 outline-none placeholder:text-white/20 focus:border-acid/40" />
      <p className="mt-2 text-right font-mono text-[10px] font-bold text-white/30">{text.length} / 180</p>
      <div className="mt-5 grid gap-4 sm:grid-cols-2"><label><span className="mono-label text-white/60">Category</span><select value={category} onChange={(event) => setCategory(event.target.value as typeof category)} className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-[#161618] px-4 text-sm font-black capitalize outline-none">{categories.map((item) => <option key={item} value={item}>{item.replaceAll("-", " ")}</option>)}</select></label><label><span className="mono-label text-white/60">Suggested energy <span className="text-white/45">optional</span></span><input maxLength={180} value={energy} onChange={(event) => setEnergy(event.target.value)} placeholder="Like a CEO hiding a raccoon" className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 text-sm font-bold outline-none placeholder:text-white/20 focus:border-hot/40" /></label></div>
      <label className="mt-4 block"><span className="mono-label text-white/60">Context for the editors <span className="text-white/45">optional</span></span><textarea maxLength={500} value={context} onChange={(event) => setContext(event.target.value)} placeholder="Why this line works, where it came from, or what makes the bit land." className="mt-2 min-h-24 w-full resize-y rounded-xl border border-white/10 bg-white/[0.04] p-4 text-sm font-bold leading-6 outline-none placeholder:text-white/20 focus:border-electric/40" /></label>
      <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-white/10 bg-white/[0.025] p-4"><input type="checkbox" required checked={rights} onChange={(event) => setRights(event.target.checked)} className="mt-1 size-4 accent-[#caff33]" /><span><b className="text-sm">I wrote this or have permission to submit it.</b><span className="mt-1 block text-xs leading-5 text-white/55">No copied movie dialogue, lyrics, personal information, targeted harassment, or private chat leaks.</span></span></label>
    ...
    <aside className="grid gap-4 lg:sticky lg:top-24"><div className="panel p-5"><Lightbulb className="size-5 text-hot" /><h2 className="mt-4 text-lg font-black">What gets featured</h2><ul className="mt-4 grid gap-3 text-sm font-bold leading-6 text-white/50"><li>• Short enough to perform in one breath-ish</li><li>• A clear comedic premise</li><li>• Flexible across several energies</li><li>• Original, not quote-shaped copyright bait</li></ul></div><div className="panel p-5"><ShieldCheck className="size-5 text-acid" /><h2 className="mt-4 text-lg font-black">Human-reviewed</h2><p className="mt-2 text-sm leading-6 text-white/45">Automation catches obvious unsafe content. An editor makes the final call before anything enters a public pack.</p><Link href="/guidelines" className="button-ghost mt-4 px-0">Read the rules <ArrowRight className="size-3.5" /></Link></div></aside>
  </div>;
}
