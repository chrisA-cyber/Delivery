...
export function SubmissionForm() {
  ...
  return <div className="grid gap-5 lg:grid-cols-[1fr_360px] lg:items-start">
    <form onSubmit={submit} className="panel-solid p-5 sm:p-8">
      ...
      <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-white/10 bg-white/[0.025] p-4"><input type="checkbox" required checked={rights} onChange={(event) => setRights(event.target.checked)} className="mt-1 size-4 accent-acid" /><span><b className="text-sm">I wrote this or have permission to submit it.</b><span className="mt-1 block text-xs leading-5 text-white/55">No copied movie dialogue, lyrics, personal information, targeted harassment, or private chat leaks.</span></span></label>
