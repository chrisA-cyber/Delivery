...
it("lets a slow poll complete instead of invalidating it with another poll", async () => {
  vi.useFakeTimers();
  const round = { token, state: "revealed", canClaim: false, community: { phase: "showcase" }, members: [] };
  let finish: ((value: unknown) => void) | undefined;
  mocks.read.mockResolvedValueOnce({ round }).mockImplementation(() => new Promise((resolve) => { finish = resolve; }));
  const { result, unmount } = renderHook(() => useRound(token));
  await act(async () => {});
  await act(async () => { await vi.advanceTimersByTimeAsync(12000); });
  expect(mocks.read).toHaveBeenCalledTimes(2);
  await act(async () => { finish?.({ round: { ...round, name: "New host state" } }); });
  expect(result.current.round?.name).toBe("New host state");
  unmount();
});
