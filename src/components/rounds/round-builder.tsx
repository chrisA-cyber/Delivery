...
export function RoundBuilder({ community = false, initialMode, initialClipId = "", initialRoleId = "", initialPromptId = "", initialEnergyId = "", initialChallengeId = "", previousToken }: {
  ...
  const [mode, setMode] = useState<GroupMode>(
    initialMode === "classic" || initialMode === "switch" || initialMode === "say-it-back"
      ? initialMode
      : initialPromptId ? "classic" : initialClipId ? "say-it-back" : "switch",
  );
  ...
  return <div className="round-builder" data-mode="social">
    ...
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><ContentControl compact allowMature={false} value={contentRating} onChange={(rating) => updatePreferences({ contentRating: rating })} disabled={creating} /><p className="text-xs leading-5 text-white/60">Mature performances stay private.</p></div>
    <form onSubmit={(event) => void create(event)} className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,1fr)]">
      <section className="min-w-0 rounded-2xl border border-white/15 bg-surface p-4 sm:p-6">
        <p className="mono-label text-hot">The challenge</p><h2 className="mt-2 text-2xl font-bold">What are we playing?</h2>
        <div className="round-mode-picker mt-4 grid grid-cols-3 gap-2" role="group" aria-label="Game mode">{([{ value: "switch", label: "Switch", description: "Same phrase. New emotion or speed.", icon: Shuffle }, { value: "classic", label: "Classic", description: "A line and an acting direction.", icon: Mic }, { value: "say-it-back", label: "Say It Back", description: "Your voice in a real scene.", icon: Clapperboard }] as const).map((item) => <button type="button" key={item.value} data-mode={item.value} onClick={() => { setMode(item.value); setQuery(""); }} disabled={creating} aria-pressed={mode === item.value} className={cn("rounded-xl border p-3 text-left", mode === item.value ? "border-hot bg-hot/10" : "border-white/15 hover:border-white/40")}><item.icon className="mb-2 size-5" /><span className="block text-xs font-bold sm:text-sm">{item.label}</span><span className="mt-1 hidden text-[11px] leading-5 text-white/60 sm:block">{item.description}</span></button>)}</div>
      ...
      <aside className="min-w-0 space-y-4 lg:sticky lg:top-24">
        <section className="rounded-2xl bg-paper p-5 text-ink sm:p-6"><p className="mono-label text-ink/55">Your room</p><h2 className="mt-2 text-2xl font-bold">{community ? "Invite your community" : "Invite your friends"}</h2>
