...
export function RoundRecording({ token, initialAttemptId, initialClaimId }: { token: string; initialAttemptId?: string; initialClaimId?: string }) {
  ...
  if (round.assignment.mode === "switch") return <SwitchExperience initialAttemptId={initialAttemptId} initialClaimId={initialClaimId} roundContext={{ token, returnPath, challenge: round.assignment.challenge }} />;
