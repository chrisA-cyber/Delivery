...
export const SwitchPlayer = forwardRef<SwitchPlayerHandle, {
  ...
  return <section className={cn("overflow-hidden rounded-2xl border border-white/15 bg-surface", className)} aria-label={`${takeLabel} with Switch cues`}>
    ...
    <div className="px-4 pb-4 sm:px-5">
      <div className={cn("rounded-xl border px-3 py-2.5", challenge.kind === "speed" ? "border-electric/25 bg-electric/[.07]" : "border-hot/25 bg-hot/[.07]")} aria-label="Current Switch direction">
        <div className="flex items-center gap-3"><span className={cn("shrink-0 font-black leading-none", challenge.kind === "speed" ? "text-4xl tabular-nums text-electric" : "text-5xl text-hot")} aria-hidden="true">{challenge.kind === "speed" ? `${cue.speed ?? 1}×` : cue.emoji}</span><div className="min-w-0 flex-1"><h3 className="text-xl font-black tracking-tight text-paper" aria-label={cue.directionLabel}>{cue.directionLabel.replace(cue.emoji, "").trim()}</h3><p className="mt-0.5 truncate text-[11px] text-white/50">{next ? `Next: ${challenge.kind === "speed" ? `${next.speed ?? 1}×` : next.emoji} ${next.directionLabel.replace(next.emoji, "").trim()}` : "Final cue"}</p></div><span className={cn("shrink-0 text-xs tabular-nums", challenge.kind === "speed" ? "text-electric" : "text-hot")}>{challenge.cues.indexOf(cue) + 1} / {challenge.cues.length}</span></div>
      </div>
      <div className="mt-2 grid gap-1" style={{ gridTemplateColumns: `repeat(${challenge.cues.length}, minmax(0, 1fr))` }} aria-label="Jump to a cue">
        {challenge.cues.map((item) => <button type="button" key={item.id} onClick={() => seek(item.start)} aria-label={`Jump to ${item.directionLabel}`} aria-current={cue.id === item.id ? "step" : undefined} title={`${item.directionLabel} · ${item.start}–${item.end}s`} className={cn("flex min-h-11 items-center justify-center rounded-lg border text-lg font-bold", cue.id === item.id ? challenge.kind === "speed" ? "border-electric/35 bg-electric/10 text-electric" : "border-hot/35 bg-hot/10 text-hot" : "border-transparent text-white/45 hover:bg-white/5 hover:text-paper")}><span aria-hidden="true">{challenge.kind === "speed" ? `${item.speed ?? 1}×` : item.emoji}</span></button>)}
      </div>
      <div className="flex min-h-32 items-center gap-3 py-3 sm:min-h-36 sm:gap-6">{camera.length ? <CameraPlayback segments={camera} clockRef={audio} onError={pause} /> : performerAvatar && <PerformerAvatar size={104} level={voiceLevel} />}<p className="min-w-0 flex-1 text-2xl font-black leading-snug tracking-tight text-paper sm:text-3xl">“{cue.text}”</p></div>
    </div>
    <div className="border-t border-white/10 px-4 pb-3 pt-2 sm:px-5"><div className="flex items-center gap-2"><button type="button" className="button-primary min-h-11 shrink-0 px-4 text-xs" onClick={() => playing ? pause() : void play()}>{playing ? <Pause className="size-4" /> : <Play className="size-4" />}{playing ? "Pause" : "Play take"}</button><label className="sr-only" htmlFor={seekId}>Seek through take</label><input id={seekId} aria-label="Seek through take" type="range" min={0} max={challenge.duration} step={0.05} value={Math.min(time, challenge.duration)} onChange={(event) => seek(Number(event.target.value))} className="h-11 min-w-0 flex-1 accent-mint" /><button type="button" className="icon-button shrink-0 border-transparent" onClick={() => void replay()} aria-label="Replay from beginning"><RotateCcw className="size-4" /></button></div>{buffering && <p role="status" className="mt-2 text-xs text-white/50">Loading audio…</p>}{error && <p className="mt-2 text-sm text-hot" role="alert">{error}</p>}</div>
  </section>;
});
