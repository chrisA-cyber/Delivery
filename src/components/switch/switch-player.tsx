...
export const SwitchPlayer = forwardRef<SwitchPlayerHandle, {
  ...
  return <section className={cn("overflow-hidden rounded-2xl border border-white/15 bg-surface", className)} aria-label={`${takeLabel} with Switch cues`}>
    ...
    <div className="p-5 sm:p-7"><p className="mono-label text-acid">{time >= challenge.duration ? "Last direction" : "Direction now"}</p><div className="mt-3 flex items-center gap-5">{performerAvatar && <PerformerAvatar size={112} level={voiceLevel} />}<h3 className="text-3xl font-black tracking-tight text-acid sm:text-4xl"><span className="mb-3 block text-6xl" aria-hidden="true">{cue.emoji}</span>{cue.directionLabel}</h3></div><p className="mt-4 text-xl font-semibold leading-relaxed text-paper">{cue.text}</p><p className="mt-4 text-xs text-white/55">{next ? `Up next: ${next.emoji} ${next.directionLabel} · at ${next.start}s` : "Bring it home. This is the final switch."}</p></div>
