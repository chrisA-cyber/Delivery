...
import { ArrowLeft, ArrowRight, ArrowUpRight, Check, Headphones, LoaderCircle, Mic, Play, Shuffle, Users, X } from "lucide-react";
...
function ScorePanel({ challenge, score, jump, replay, retry, retryDisabled }: { challenge: SwitchChallenge; score: SwitchScore; jump: (time: number) => void; replay: () => void; retry: () => void; retryDisabled: boolean }) {
  const result = useRef<HTMLElement>(null);
  useEffect(() => {
    result.current?.focus({ preventScroll: true });
    result.current?.scrollIntoView({ block: "start", behavior: "instant" });
  }, [score]);
  return <section ref={result} tabIndex={-1} className="scroll-mt-[88px] overflow-hidden rounded-2xl border border-mint/20 bg-paper text-ink outline-none" aria-label="Your Switch result">
    <div className="p-5 sm:p-6">
      <div className="flex items-center justify-between gap-3"><p className="text-xs font-bold">Your Switch</p><span className="rounded-full border border-ink/15 px-2 py-1 text-[10px] font-bold text-ink/60">Beta result</span></div>
      <div className="mt-3 flex items-center gap-4"><p className="display-type shrink-0 text-7xl leading-none">{score.overall == null ? "—" : Math.round(score.overall)}<span className="font-sans text-sm font-bold tracking-normal text-ink/50">{score.overall == null ? "" : "/100"}</span></p><div className="min-w-0"><h2 className="text-xl font-bold leading-tight">{score.overall == null ? "Here’s what we heard." : score.overall >= 75 ? "You made the switch." : "Try one more take."}</h2><p className="mt-2 text-sm leading-5 text-ink/70">{score.coachNote}</p></div></div>
      <div className="mt-5 grid grid-cols-3 gap-2">{[{ name: "Words", value: score.words }, { name: "Directions", value: score.delivery }, { name: "Switches", value: score.transitions }].map((item) => <div key={item.name} className="rounded-xl border border-ink/10 bg-white/40 p-3"><p className="text-[11px] font-bold text-ink/55">{item.name}</p><p className="mt-1 text-2xl font-bold">{item.value == null ? "—" : Math.round(item.value)}</p></div>)}</div>
      <div className="mt-4 flex flex-wrap gap-2"><button type="button" className="button-primary min-h-11" disabled={retryDisabled} onClick={retry}><Mic className="size-4" />Another take</button><button type="button" className="button-secondary min-h-11 !border-ink/20 !bg-transparent !text-ink" onClick={replay}><Play className="size-4" />Replay take</button></div>
      <details className="mt-3 border-t border-ink/10 pt-1"><summary className="min-h-11 cursor-pointer py-3 text-sm font-bold">Hear each switch</summary><p className="mb-3 text-sm leading-5 text-ink/70">{score.transitionFeedback}</p><div className="space-y-2">{score.segments.map((segment) => { const cue = challenge.cues.find((item) => item.id === segment.cueId); return <button type="button" key={segment.cueId} className="flex w-full items-start gap-3 rounded-xl border border-ink/15 p-3 text-left hover:bg-ink/5" onClick={() => jump(cue?.start ?? 0)}><Headphones className="mt-0.5 size-4 shrink-0" /><span><span className="block text-sm font-bold">{cue?.directionLabel} <span className="font-normal text-ink/50">· {cue?.start}s</span></span><span className="mt-1 block text-xs leading-5 text-ink/70">{segment.feedback}</span></span></button>; })}</div></details>
      <details className="border-t border-ink/10 text-xs leading-5 text-ink/65"><summary className="min-h-11 cursor-pointer py-3">About this result</summary><p>A casual audio assessment of your words, directions and transitions. Quiet acting can score well. Results are unranked.</p>{score.limitations.map((note, index) => <p className="mt-2" key={index}>{note}</p>)}<p className="mt-2">Compare takes of the same challenge and timing. Your original recording is unchanged.</p><p className="mt-3 font-bold">Heard dialogue</p><p>{score.transcript || "No reliable transcript."}</p></details>
    </div>
  </section>;
}
...
export function SwitchExperience({ initialChallengeId, initialAttemptId, initialClaimId, challengeToken, roundContext, publicAssignment }: { initialChallengeId?: string; initialAttemptId?: string; initialClaimId?: string; challengeToken?: string; roundContext?: SwitchRoundContext; publicAssignment?: { code: string; assignment: Extract<GroupAssignment, { mode: "switch" }> } }) {
  ...
  const recorder = useAudioRecorder({ mode: "switch" });
  ...
  const captureBusy = finalizing || recording || preparing || countdown != null;
  ...
  const submissionReady = canSubmit && (!invitation || consent || !!attempt);
  const allowed = !challenge || isRatingAllowed(challenge.rating, contentRating);
  const currentTime = recording || finalizing ? recorder.durationMs / 1000 : 0;
  ...
  const featuredPhrase = phrases.find((phrase) => phrase.variants.emotion?.id === "literally") ?? phrases[0];
  ...
  useEffect(() => {
    ...
    if (recorder.audioBlob && recorder.audioBlob !== pendingCapture.current.previousBlob) {
      ...
      commitCapture(); setNotice(recorder.stopReason === "limit" ? "Take complete. Ready to replay." : "Stopped early. Replay it, then try the full take.");
    }
  ...
  const cancel = () => { countdownRun.current++; pendingCapture.current = null; cancelCapture(); setCountdown(null); setPreparing(false); inFlight.current = false; setNotice(audioUrl ? "Cancelled. Your previous take is still here." : "Cancelled. Ready when you are."); };
  const cameraSegments = useMemo(() => recorder.audioUrl ? cameraInterval(recorder.cameraTake, 0, recorder.durationMs / 1000) : undefined, [recorder.audioUrl, recorder.cameraTake, recorder.durationMs]);
  const upload = async (run: number): Promise<SwitchAttempt> => {
    if (attempt) { await saveCamera("switch", attempt.id, cameraSegments ?? [], contentRating); return attempt; }
    ...
    await saveCamera("switch", saved.id, cameraSegments ?? [], contentRating);
  ...
  const performAction = async (action: "judge" | "save" | "signin" | "round" | "share") => {
    ...
    if (action === "share" && !consent) { setError("Confirm sharing your recording before creating an invitation."); return; }
    ...
    try {
      ...
      if (action === "judge") {
        ...
        setNotice("Result ready. Open “Hear each switch” for a closer listen.");
  ...
  return <main className="min-h-screen px-4 pb-20 pt-20 sm:pt-24"><div className="mx-auto max-w-6xl"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><Link href={roundContext?.returnPath ?? "/"} className="button-ghost -ml-3 min-h-11 min-w-11 px-3 text-xs"><ArrowLeft className="size-4" /><span className="sr-only sm:not-sr-only">{roundContext ? "Back to round" : "All modes"}</span></Link><ContentControl value={contentRating} onChange={(value) => updatePreferences({contentRating:value})} disabled={captureBusy || !!busy} compact /></div>
    {loading ? <div className="panel-solid p-8 text-center"><LoaderCircle className="mx-auto size-7 animate-spin text-acid" /><p className="mt-4">Getting Switch ready…</p></div> : loadError ? <div className="panel-solid p-8"><h1 className="text-2xl font-bold">Couldn’t open Switch.</h1><p role="alert" className="mt-4 text-white/65">{loadError}</p><button className="button-primary mt-5" onClick={() => void load()}>Try again</button></div> : !challenge ? <>
      <header className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h1 className="display-type text-4xl sm:text-5xl">Switch.</h1><p className="mt-1 text-sm text-white/60">Same phrase. Five emotions or speeds.</p></div><Link href="/rounds?mode=switch" className="button-ghost min-h-11 px-2 text-xs"><Users className="size-4" /><span className="sm:hidden">Friends</span><span className="hidden sm:inline">Play with friends</span></Link></header>
      {featuredPhrase && <SwitchPhraseCard featured phrase={featuredPhrase} onChoose={choose} />}
      <div className="mb-3 flex items-center justify-between gap-3"><h2 className="text-sm font-bold">{phrases.length > 1 ? "More phrases" : "Your phrase"}</h2><p className="text-xs text-white/45" role="status">{phrases.length} {phrases.length === 1 ? "phrase" : "phrases"}</p></div>
      <div className="grid gap-2 lg:grid-cols-2">{phrases.filter((phrase) => phrase.key !== featuredPhrase?.key).map((phrase) => <SwitchPhraseCard compact key={phrase.key} phrase={phrase} onChoose={choose} />)}</div>{phrases.length === 0 && <p className="rounded-xl border border-white/15 p-5 text-sm text-white/65">No phrases match this content setting.</p>}
      ...
      <div className={cn("grid items-start gap-4", audioUrl && !captureBusy ? "lg:grid-cols-[1.1fr_1fr]" : "mx-auto max-w-3xl")}>
        <section ref={stage} className="switch-stage scroll-mt-20 overflow-hidden rounded-2xl border border-white/15 bg-[var(--ink-soft)]" data-recording={captureBusy} data-kind={challenge.kind} aria-label="Switch recording stage">
          {!captureBusy && <div className="flex flex-wrap items-center justify-between gap-2 px-4 pt-3"><CameraControls camera={recorder.camera} disabled={Boolean(busy)} />{audioUrl && <span className="text-[10px] text-white/45">For your next take</span>}</div>}
          {audioUrl && !captureBusy ? <SwitchPlayer cameraSegments={cameraSegments} performerAvatar ref={player} key={audioUrl} challenge={challenge} audioUrl={audioUrl} className="!rounded-none !border-0 !bg-transparent" /> : <div className="p-4 sm:p-5">
            <div className={cn("rounded-xl border px-3 py-3", challenge.kind === "speed" ? "border-electric/25 bg-electric/[.07]" : "border-hot/25 bg-hot/[.07]")} aria-label="Current Switch direction">
              <div className="flex items-center gap-3"><span className={cn("shrink-0 font-black leading-none", challenge.kind === "speed" ? "text-4xl tabular-nums text-electric" : "text-5xl text-hot")} aria-hidden="true">{challenge.kind === "speed" ? `${cue?.speed ?? 1}×` : cue?.emoji}</span><div className="min-w-0 flex-1"><h2 className="text-xl font-black text-paper sm:text-2xl" aria-label={cue?.directionLabel} aria-live={recording ? "polite" : "off"}>{cue?.directionLabel.replace(cue.emoji, "").trim()}</h2><p className="mt-1 text-[11px] text-white/50">{recording ? `Switch ${challenge.cues.indexOf(cue!) + 1} of ${challenge.cues.length} · say it now` : "Say it once. Wait for the next cue."}</p></div><div className="shrink-0 text-right">{countdown != null && countdown > 0 ? <p className="display-type text-5xl text-acid" role="status">{countdown}</p> : <><p className={cn("text-xs font-bold", recording ? "text-hot" : "text-white/45")}>{recording ? "● REC" : finalizing ? "Finishing…" : preparing ? "Starting mic…" : "Start with"}</p>{recording && <p className="mt-1 text-xs tabular-nums text-white/60">{Math.max(0, challenge.duration - currentTime).toFixed(1)}s</p>}</>}</div></div>
            </div>
            <ol className="mt-2 grid gap-1" style={{ gridTemplateColumns: `repeat(${challenge.cues.length}, minmax(0, 1fr))` }} aria-label="Switch cue order">{challenge.cues.map((item) => <li key={item.id} title={`${item.directionLabel} · ${item.start}–${item.end}s`} aria-current={cue?.id === item.id ? "step" : undefined} className={cn("flex min-h-11 items-center justify-center rounded-lg border text-lg font-bold", cue?.id === item.id ? challenge.kind === "speed" ? "border-electric/35 bg-electric/10 text-electric" : "border-hot/35 bg-hot/10 text-hot" : "border-transparent text-white/45")}><span aria-hidden="true">{challenge.kind === "speed" ? `${item.speed ?? 1}×` : item.emoji}</span><span className="sr-only">{item.directionLabel}, {item.start} to {item.end} seconds</span></li>)}</ol>
            <div className="flex min-h-32 items-center gap-3 py-3 sm:min-h-40 sm:gap-6"><div className="shrink-0"><CameraMonitor camera={recorder.camera} size={104} level={recording ? recorder.voiceLevel : 0} editable={!captureBusy} /></div><p className="min-w-0 flex-1 text-2xl font-black leading-snug tracking-tight text-paper sm:text-3xl">“{cue?.text}”</p></div>
            <div className="flex items-center gap-3"><p className="min-w-0 flex-1 truncate text-xs text-white/50">{nextCue ? `Next: ${challenge.kind === "speed" ? `${nextCue.speed ?? 1}×` : nextCue.emoji} ${nextCue.directionLabel.replace(nextCue.emoji, "").trim()}` : "Final cue"}</p><div role="meter" aria-label="Microphone level" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(recorder.level * 100)} className="flex w-24 shrink-0 items-center gap-0.5">{Array.from({ length: 16 }, (_, index) => <span key={index} className={cn("h-3 flex-1 rounded-sm", index / 16 < recorder.level ? "bg-mint" : "bg-white/10")} />)}</div></div>
            {captureBusy && <div className="mt-3 h-1 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-mint" style={{ width: `${Math.min(100, currentTime / challenge.duration * 100)}%` }} /></div>}
          </div>}
          <div className="border-t border-white/10 px-4 py-3 sm:px-5">
            {captureBusy ? <div className="flex flex-wrap gap-2"><button className="button-secondary min-h-11" onClick={cancel}><X className="size-4" />Cancel take</button>{recording && <button className="button-ghost min-h-11" onClick={stop}>Stop early</button>}</div> : <button type="button" className={cn("min-h-11 w-full", audioUrl ? "button-secondary" : "button-primary")} disabled={!!busy && busy !== "judge"} aria-label={audioUrl ? "Retry full take" : "Record Switch"} onClick={() => void begin()}><Mic className="size-4" />{audioUrl ? "Another take" : "Record Switch"}</button>}
            {!captureBusy && <div className="mt-1 flex flex-wrap items-center justify-between gap-x-3 text-[11px] leading-5 text-white/45">{!audioUrl && <p>3-second countdown · stops automatically</p>}<details className="max-w-sm"><summary className="min-h-11 cursor-pointer py-3 text-white/60">Tips</summary><p className="mt-2">{challenge.kind === "speed" ? "Say the same line at each requested pace. Your recording stays at its original speed." : "Say the same line in each emotion. Follow the next cue when it changes."} Replay and retry as often as you like.</p><p className="mt-2">The optional beta judge listens for words, directions and transitions. Results are casual and unranked.</p>{phraseSource && <a href={phraseSource} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex min-h-11 items-center gap-1 text-white/60 hover:text-paper">Phrase inspiration <ArrowUpRight className="size-3" /></a>}</details></div>}
            {recorder.error && <p role="alert" className="mt-2 text-sm text-hot">{recorder.error}</p>}
          </div>
        </section>
      <div className="space-y-3 empty:hidden">{audioUrl && !captureBusy ? <>
        {!canSubmit && <p className="rounded-xl bg-hot/10 p-4 text-sm text-hot">{recorder.qualityMessage} Finish all {challenge.cues.length} cues to save or share.</p>}
        {recorder.quality === "quiet" && !attempt && <p className="text-sm text-white/60">Quiet take? Check that you can hear the words in replay.</p>}
        {recorder.warning && recorder.stopReason !== "limit" && <p className="text-sm text-hot">{recorder.warning}</p>}
        {invitation && !attempt && <label className="flex items-start gap-3 rounded-xl border border-violet/25 bg-violet/5 p-4 text-sm leading-5 text-white/70"><input className="mt-1 size-4 accent-violet" type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} />Let my friend see and hear this response, including my camera when recorded, and compare our beta results.</label>}
        {attempt?.score && <ScorePanel challenge={challenge} score={attempt.score} retry={() => void begin()} retryDisabled={!!busy && busy !== "judge"} replay={() => { stage.current?.scrollIntoView({ block: "start", behavior: "instant" }); void player.current?.replay(); }} jump={(time) => { stage.current?.scrollIntoView({ block: "start", behavior: "instant" }); player.current?.seek(time); void player.current?.play(); }} />}
        <section className="rounded-2xl border border-white/10 bg-surface p-4 sm:p-5" aria-label="Take actions">
          {!attempt?.score && <div className="mb-3"><h2 className="font-bold">{canSubmit ? "How did you switch?" : "Keep this one for practice."}</h2><p className="mt-1 text-xs leading-5 text-white/55">{canSubmit ? "Replay your take. Get a result or make a video." : "Another full take unlocks results and video."}</p></div>}
          <div className="flex flex-wrap gap-2">
            {roundContext && <button className="button-primary min-h-11" disabled={!!busy || !submissionReady} onClick={() => void performAction("round")}><Users className="size-4" />Use this take in round</button>}
            {attempt?.status !== "scored" && <button className={cn("min-h-11", roundContext ? "button-secondary" : "button-primary")} disabled={!!busy || !submissionReady} onClick={() => void performAction("judge")}>{busy === "judge" || busy === "upload" ? <LoaderCircle className="size-4 animate-spin" /> : <Shuffle className="size-4" />}{busy === "judge" ? "Listening to your switches…" : attempt?.status === "failed" ? "Retry scoring" : "Judge my Switch"}</button>}
            {(!attempt || attempt.owned) && <div className="[&>section]:!mt-0"><VideoExport compact key={`export:${audioUrl}`} mode="switch" attemptId={attempt?.id} prepareAttempt={prepareVideoExport} hasScore={attempt?.score?.overall != null} disabled={!!busy || !submissionReady} /></div>}
            <button className="button-ghost min-h-11" disabled={!!busy || !submissionReady} onClick={() => void performAction(authenticated ? "save" : "signin")}>{authenticated ? <Check className="size-4" /> : <ArrowRight className="size-4" />}{authenticated ? attempt?.saved ? "Saved privately" : "Save take" : "Sign in & keep take"}</button>
          </div>
          {busy === "judge" && <p role="status" className="mt-3 text-xs leading-5 text-white/60">You can keep replaying while we listen.</p>}
          {remaining !== null && <p className="mt-3 text-[11px] text-white/50">{remaining} judged plays left today · practice is free.</p>}
          {attempt?.warning && <p className="mt-3 text-xs leading-5 text-white/55">{attempt.warning}</p>}
        </section>
        {!roundContext && !invitation && attempt?.score && <details className="rounded-2xl border border-violet/25 bg-violet/[.05] px-4 sm:px-5"><summary className="min-h-12 cursor-pointer py-4 text-sm font-bold text-violet">Challenge a friend</summary><div className="pb-4">
          {authenticated ? <><label className="flex items-start gap-3 text-xs leading-5 text-white/65"><input type="checkbox" className="mt-1 size-4 accent-violet" checked={consent} onChange={(event) => setConsent(event.target.checked)} />Share my voice and recorded camera with people who open this invitation.</label><button className="button-secondary mt-3 min-h-11" disabled={!!busy || !consent} onClick={() => void performAction("share")}><Users className="size-4" />Create friend challenge</button></> : <><p className="text-xs leading-5 text-white/65">Sign in to share your take and compare the same phrase, cues and timing.</p><button className="button-secondary mt-3 min-h-11" disabled={!!busy} onClick={() => void performAction("signin")}><Users className="size-4" />Sign in & challenge</button></>}
          {createdLink && <div className="mt-3"><input className="input-field w-full text-xs" aria-label="Friend challenge link" value={createdLink} readOnly onFocus={(event) => event.target.select()} /><button className="button-ghost mt-2" onClick={() => void navigator.clipboard.writeText(createdLink).then(() => setNotice("Invitation copied.")).catch(() => setNotice("Select and copy the invitation above."))}>Copy link</button></div>}
        </div></details>}
      </> : null}
      {error && <p role="alert" className="rounded-xl border border-hot/25 bg-hot/10 p-4 text-sm leading-5 text-hot">{error}</p>}
      {notice && <p role="status" className="rounded-xl border border-mint/20 bg-mint/5 px-4 py-3 text-xs leading-5 text-white/75">{notice}</p>}
      </div></div>
      {invitation && <section className="mt-8 space-y-4"><h2 className="text-xl font-bold">Your friend’s challenge</h2>{[invitation.challengerAttempt, ...(invitation.recipientAttempts ?? [])].filter((item): item is SwitchAttempt => !!item).map((item)=><div key={item.id} className="max-w-xl"><SwitchPlayer challenge={item.challenge} audioUrl={item.audioUrl} takeLabel={item.owned ? "Your response" : "Friend’s take"} />{item.score && <p className="mt-2 text-sm text-white/60">Beta result: {item.score.overall ?? "Uncertain"}{item.score.overall != null ? "/100" : ""}</p>}</div>)}</section>}
    </>}
  </div></main>;
}
