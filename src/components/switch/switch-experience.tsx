...
export function SwitchExperience({ initialChallengeId, initialAttemptId, initialClaimId, challengeToken, roundContext, publicAssignment }: { initialChallengeId?: string; initialAttemptId?: string; initialClaimId?: string; challengeToken?: string; roundContext?: SwitchRoundContext; publicAssignment?: { code: string; assignment: Extract<GroupAssignment, { mode: "switch" }> } }) {
  ...
  return <main className="min-h-screen px-4 pb-28 pt-28 sm:pt-32"><div className="mx-auto max-w-6xl"><div className="mb-7 flex flex-wrap items-center justify-between gap-3"><Link href={roundContext?.returnPath ?? "/"} className="button-ghost -ml-3 px-3"><ArrowLeft className="size-4" />{roundContext ? "Back to round" : "All modes"}</Link><ContentControl value={contentRating} onChange={(value) => updatePreferences({contentRating:value})} disabled={captureBusy || !!busy} compact /></div>
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{catalog.map((item) => <button key={item.id} type="button" onClick={() => choose(item)} className="group flex flex-col rounded-2xl border border-white/15 bg-[var(--ink-soft)] p-5 text-left transition-colors hover:border-hot/60 sm:p-6">
