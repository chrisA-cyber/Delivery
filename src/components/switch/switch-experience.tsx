...
export function SwitchExperience({ initialChallengeId, initialAttemptId, initialClaimId, challengeToken, roundContext }: { initialChallengeId?: string; initialAttemptId?: string; initialClaimId?: string; challengeToken?: string; roundContext?: SwitchRoundContext }) {
  ...
  const [challenge, setChallenge] = useState<SwitchChallenge | null>(roundContext?.challenge ?? null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let selected = roundContext?.challenge ?? data.challenges.find((item) => item.id === initialChallengeId) ?? null;
  ...
  }, [authReady, authenticated, contentRating, initialChallengeId, initialAttemptId, initialClaimId, challengeToken, roundContext]);
  ...
  const performAction = async (action: "judge" | "save" | "signin" | "round" | "share") => {
    ...
    try {
      ...
      if (action === "judge") {
        ...
        const path = roundContext ? `/rounds/${encodeURIComponent(roundContext.token)}/record` : "/switch";
