...
export function SwitchExperience({ initialChallengeId, initialAttemptId, initialClaimId, challengeToken, roundContext, publicAssignment }: { initialChallengeId?: string; initialAttemptId?: string; initialClaimId?: string; challengeToken?: string; roundContext?: SwitchRoundContext; publicAssignment?: { code: string; assignment: Extract<GroupAssignment, { mode: "switch" }> } }) {
  ...
  return <main className="min-h-screen px-4 pb-28 pt-28 sm:pt-32"><div className="mx-auto max-w-6xl"><div className="mb-7 flex flex-wrap items-center justify-between gap-3"><Link href={roundContext?.returnPath ?? "/"} className="button-ghost -ml-3 px-3"><ArrowLeft className="size-4" />{roundContext ? "Back to round" : "All modes"}</Link><ContentControl value={contentRating} onChange={(value) => updatePreferences({contentRating:value})} disabled={captureBusy || !!busy} compact /></div>
    {loading ? <div className="panel-solid p-8 text-center"><LoaderCircle className="mx-auto size-7 animate-spin text-acid" /><p className="mt-4">Getting Switch ready…</p></div> : loadError ? <div className="panel-solid p-8"><h1 className="text-2xl font-bold">Couldn’t open Switch.</h1><p role="alert" className="mt-4 text-white/65">{loadError}</p><button className="button-primary mt-5" onClick={() => void load()}>Try again</button></div> : !challenge ? <><header className="mb-6 max-w-3xl">
      <p className="mono-label flex items-center gap-2 text-hot"><Shuffle className="size-4" />Emoji or speed</p>
      <h1 className="display-type mt-3 text-5xl sm:text-6xl">Switch.</h1>
      <p className="mt-3 text-base leading-7 text-white/70">Pick a phrase. Play it with emojis or changing speeds.</p>
    </header>
    <div className="mb-4 flex items-center justify-between gap-3"><h2 className="text-lg font-bold">Choose a phrase</h2><p className="text-sm text-white/50" role="status">{phrases.length} {phrases.length === 1 ? "phrase" : "phrases"}</p></div>
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{phrases.map((phrase) => <SwitchPhraseCard key={phrase.key} phrase={phrase} onChoose={choose} />)}</div>{phrases.length === 0 && <p className="rounded-2xl border border-white/15 p-6 text-sm text-white/65">No phrases match this content setting.</p>}<Link href="/rounds?mode=switch" className="button-ghost mt-6"><Users className="size-4" />Play Switch with friends</Link></> : !allowed ? <div className="panel-solid p-8"><h1 className="text-2xl font-bold">This Switch needs a different content setting.</h1><p className="mt-3 text-white/65">Use the content control above to open the assignment.</p></div> : <>
      ...
      {selectedPhrase && !roundContext && !publicAssignment && !invitation && !audioUrl && <div className="mb-6 max-w-sm"><SwitchModePicker phrase={selectedPhrase} selectedId={challenge.id} onChoose={choose} disabled={captureBusy || !!busy} /></div>}
