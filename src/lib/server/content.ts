...
function available(from: string | null, until: string | null): boolean {
  const now = Date.now();
  return (!from || new Date(from).getTime() <= now) &&
...
export function resolvePromptPackEntitlement(
  ...
  const activeMemberships = memberships.filter(
    (pack) =>
      pack.state === "published" && available(pack.available_from, pack.available_until),
  );
  ...
  return {
    requiresPro:
      mode === "stream" ||
      mode === "impossible" ||
      (mode !== "daily" && activeMemberships.every((pack) => pack.access === "pro")),
  };
}
...
function assertPremium(content: CanonicalDeliveryContent, usage: JudgingUsage): void {
...
function resolveLocal(input: ResolveContentInput): CanonicalDeliveryContent {
  ...
  const energy =
    ...
    ENERGY_MODIFIERS.find(
      (candidate) => normalized(candidate.instruction) === normalized(input.energy),
    );
  if (!energy) {
    throw new AppError(
      "ENERGY_NOT_FOUND",
  ...
  const difficulty = { easy: 1, medium: 2, hard: 3, impossible: 4 }[prompt.difficulty];
  ...
  if (input.mode === "daily") {
    ...
    if (daily.prompt.id !== prompt.id || daily.energy.id !== energy.id || dailyMarket !== "global") {
      throw new AppError(
...
async function resolveDatabase(input: ResolveContentInput): Promise<CanonicalDeliveryContent> {
  ...
  let query = supabase
    .from("prompts")
    .select(
      "id,slug,body,category,difficulty,state,available_from,available_until,pack_prompts(content_packs(access,state,available_from,available_until))",
    )
    .limit(1);
  ...
  if (promptError) throw new ExternalServiceError("Supabase content", { cause: promptError });
  ...
  let energyQuery = supabase
    ...
    .select("id,slug,instruction,state")
    .limit(1);
  energyQuery = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(
    input.energy,
  )
  ...
  const { data: rawEnergy, error: energyError } = await energyQuery.maybeSingle();
  if (energyError) throw new ExternalServiceError("Supabase content", { cause: energyError });
  ...
  if (input.challengeId) {
    ...
    const { data: challenge, error: challengeError } = await supabase
      ...
      .maybeSingle();
    if (challengeError) {
      throw new ExternalServiceError("Supabase challenge", { cause: challengeError });
    }
    if (!challenge) {
      throw new AppError(
        "CHALLENGE_INVALID",
    ...
    const hasInviteToken = validChallengeToken(challenge.token_digest, input.challengeToken);
    let valid =
      challenge &&
      ...
      challenge.prompt_id === prompt.id &&
      (!challenge.energy_modifier_id || challenge.energy_modifier_id === energy.id);
    if (
      valid &&
    ...
    if (valid) {
      const counterpartIds = [challenge.created_by, challenge.recipient_user_id]
        .filter((id): id is string => typeof id === "string" && id !== challengeUser.id);
      for (const counterpartId of counterpartIds) {
        const { data: block, error: blockError } = await supabase
          ...
          .maybeSingle();
        if (blockError) {
          throw new ExternalServiceError("Supabase challenge privacy", { cause: blockError });
        }
        if (block) {
          valid = false;
          break;
    ...
    if (entriesError) {
      throw new ExternalServiceError("Supabase challenge entries", { cause: entriesError });
    }
  ...
  if (input.mode === "daily") {
    ...
    const { data: daily, error: dailyError } = await supabase
      ...
      .maybeSingle();
    if (dailyError) {
      throw new ExternalServiceError("Supabase daily challenge", { cause: dailyError });
    }
    if (
      !daily ||
