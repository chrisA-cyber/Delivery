...
import { resolveExportSource, checkedExport, exportUnavailable, type ExportMode } from "@/lib/server/video-export-sources";
export const VIDEO_LAYOUT_VERSION = "delivery-vertical-v1";
...
export function presentVideoExport(row: Row) {
  ...
  return {id:String(row.id),status,includeScore:row.include_score === true,includeName:row.include_name === true,
...
export async function requestVideoExport(input:{mode:ExportMode;attemptId:string;includeName:boolean;includeScore:boolean;maxRating:ContentRating},viewer:SwitchViewer) {
  const source = await resolveExportSource(input.mode,input.attemptId,viewer,input.maxRating,{...input,invitation:true});
  const hash = createHash("sha256").update(JSON.stringify({layout:VIDEO_LAYOUT_VERSION,input:source.input,includeScore:input.includeScore,includeName:input.includeName})).digest("hex");
  const result = await createSupabaseAdminClient().rpc("request_video_export",{p_source_kind:source.kind,p_attempt_id:input.attemptId,p_owner_key:source.ownerKey,p_user_id:source.userId,p_mode:input.mode,p_input:source.input,p_input_hash:hash,p_layout_version:VIDEO_LAYOUT_VERSION,p_include_score:input.includeScore,p_include_name:input.includeName});
  if(result.error){
