...
}, "Use a stable HTTPS media asset or /media/say-it-back path.");
const id = z.string().regex(/^[a-z0-9][a-z0-9-]{0,79}$/);
const interval = z.object({ start: z.number().min(0).max(30), end: z.number().positive().max(30) }).refine((value) => value.end > value.start);
export const sayClipSchema = z.object({
  id, version: id, title: z.string().min(2).max(100), description: z.string().min(5).max(500),
  duration: z.number().min(1).max(25), difficulty: z.enum(["easy", "medium", "hard"]),
  ...
  roles: z.array(z.object({ id, name: z.string().min(1).max(80), description: z.string().max(300), muteIntervals: z.array(interval).min(1).max(20), dubAudioUrl: asset.optional() })).min(1).max(4),
  cues: z.array(z.object({ id, roleId: id, text: z.string().min(1).max(500), start: z.number().min(0), end: z.number().positive() })).min(1).max(40),
  source: z.object({ title: z.string().min(1).max(200), creator: z.string().min(1).max(200), url: z.string().url(), license: z.string().min(2).max(200), licenseUrl: z.string().url(), attribution: z.string().min(2).max(1000), reuseNote: z.string().min(10).max(1500), excerptStart: z.number().min(0), excerptEnd: z.number().positive() }),
