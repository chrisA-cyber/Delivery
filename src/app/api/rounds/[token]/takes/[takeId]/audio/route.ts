...
export async function GET(request: Request, context: { params: Promise<{ token: string; takeId: string }> }) {
  const requestId = requestIdFrom(request);
  try {
    const params = await context.params;
    const token = groupTokenSchema.parse(params.token); const id = z.string().uuid().parse(params.takeId);
    const maxRating = groupRatingSchema.parse(new URL(request.url).searchParams.get("maxRating") ?? "everyone");
    return await groupAudioResponse(token, await getGroupViewer(request), request, { takeId: id }, maxRating);
