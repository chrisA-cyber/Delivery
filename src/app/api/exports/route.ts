...
const options=source.extend({includeName:z.boolean().default(true),includeScore:z.boolean().default(true)}).strict();
export async function POST(request:Request){const requestId=requestIdFrom(request);try{
  assertSameOrigin(request);assertJsonRequest(request);assertContentLength(request,2048);
  const input=options.parse(await request.json());const viewer=await getSwitchViewer(request);
