...
import { createSayAttempt, getSayHistory, getSayViewer } from "@/lib/server/say-it-back";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await enforceRateLimit(getClientKey(request, "say-upload"), { limit: 15, windowMs: 10 * 60_000 });
    const form = await request.formData();
    const audio = form.get("audio");
    if (!(audio instanceof File) || audio.size < 512) throw new AppError("AUDIO_REQUIRED", "Record your dialogue first.", 422);
    ...
    const viewer = await getSayViewer(request, fields.attemptId); cookie = viewer.setCookie;
    const result = await createSayAttempt({ ...fields, audio }, viewer);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return jsonOk({ attempts: await getSayHistory(viewer.user.id, maxRating) }, requestId, { headers: { "Cache-Control": "private, no-store" } });
