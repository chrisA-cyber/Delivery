...
import { getSayChallenge, getSayViewer } from "@/lib/server/say-it-back";
...
export async function GET(request: Request, context: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    return jsonOk({ challenge: await getSayChallenge(token, await getSayViewer(request), origin, maxRating) }, requestId, { headers: { "Cache-Control": "private, no-store", "Referrer-Policy": "no-referrer" } });
