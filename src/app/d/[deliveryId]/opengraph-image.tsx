...
export default async function DeliveryOpenGraphImage({ params }: { params: Promise<{ deliveryId: string }> }) {
  ...
  return new ImageResponse(
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", gap: 24, background: VISUAL_THEME.ink, color: VISUAL_THEME.paper, fontFamily: "Arial, sans-serif", padding: 40 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 26, fontWeight: 900 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><BrandMark size={40} /><span style={{ letterSpacing: -1.5 }}>delivery</span></div><span style={{ color: VISUAL_THEME.blue, fontSize: 16 }}>{verdict}</span></div>
      <div style={{ display: "flex", flex: 1, gap: 32, alignItems: "center", borderRadius: 18, background: VISUAL_THEME.paper, color: VISUAL_THEME.ink, padding: 32 }}>
        ...
        <div style={{ display: "flex", width: 210, height: 230, flexShrink: 0, flexDirection: "column", alignItems: "center", justifyContent: "center", borderRadius: 14, background: VISUAL_THEME.accent }}><div style={{ fontSize: 100, lineHeight: 1, fontWeight: 900 }}>{score}</div><div style={{ marginTop: 12, fontSize: 20 }}>/ 100</div></div>
      </div>
