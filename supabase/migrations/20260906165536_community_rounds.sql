-- Community rounds extend the existing challenge, member, take and vote records.
-- Existing rounds default to private; consent is never inferred from old shares.
alter table public.challenges
  add column community boolean not null default false,
  add column community_phase text not null default 'submissions' check (community_phase in ('submissions','review','showcase','voting','results')),
  add column community_limit integer not null default 25 check (community_limit between 10 and 50),
  add column community_voting boolean not null default true,
  add column community_code text unique,
  add column display_token_hash text unique,
  add column display_revoked_at timestamptz,
  add column display_member_id uuid references public.challenge_group_members(id) on delete set null,
  add column display_command text not null default 'pause' check (display_command in ('play','pause','replay')),
  add column display_revision integer not null default 0,
  add column community_next_id uuid references public.challenges(id) on delete set null;
alter table public.challenge_group_takes add column broadcast_consent_at timestamptz;
alter table public.challenge_group_members add column showcased boolean not null default false;
...
$$;
drop function public.create_group_round(uuid,text,text,text,jsonb,timestamptz,uuid,uuid,text);
...
revoke all on function public.create_group_round(uuid,text,text,text,jsonb,timestamptz,uuid,uuid,text,boolean,integer,boolean,text) from public,anon,authenticated;
grant execute on function public.create_group_round(uuid,text,text,text,jsonb,timestamptz,uuid,uuid,text,boolean,integer,boolean,text) to service_role;
