...
async function asset(url:string,integrity:Record<string,string>|undefined,scratch:string,signal:AbortSignal){
 const custom=/^\/api\/say-it-back\/imports\/([a-f0-9-]{36})\/media\/(video|poster|reference|backing)\?key=([a-f0-9]{48})$/.exec(url);
 if(custom){
  const row=await loadSayImport(custom[1]!);const kind=custom[2] as SayImportAsset;
  if(url!==sayImportMediaUrl(row,kind))throw new Error("SCENE_ASSET_UNAVAILABLE");
  const file=path.join(scratch,`scene-${kind}`);await downloadImportAsset(row,kind,file,signal);
  const expected=integrity?.[url];
  if(!expected||createHash("sha256").update(await readFile(file)).digest("hex")!==expected)throw new Error("SCENE_ASSET_CHANGED");
  return file;
 }
...
async function run(job:Row){
 ...
 const timeout=setTimeout(()=>controller.abort(),(job.input as unknown as ExportInput).assignment.mode==="say-it-back"?480_000:300_000);
...
while(!stopping){
 try{
  ...
  try{if(await runNextSayImport(current.signal))continue;}finally{current=null;}
