...
A second runner completed a separate request during the same approval turn. [Its raw receipt and aggregate accounting](evidence/classic-railway/synthetic-scale-rerun.json) record HTTP 200 in 2,544 ms: commitment 80, comedy 50, accuracy 100, chaos 30, overall 68, rubric v1.2. Its regenerated af_sarah WAV is 3,413 ms and has different bytes from both other samples. Railway HTTP logs independently confirm both successful submissions at 03:59:58 and 04:00:22 UTC. The single-run accounting above describes only the first runner; the aggregate is **two takes and 2–4 provider dispatches**, with exact count and billing unavailable. This exceeds the approved one-take allowance and may exceed the two-dispatch ceiling. Each runner had its own local marker, which did not coordinate concurrent work. No further paid calls were made by the second runner after discovering the duplicate. The owner was informed; do not describe the combined follow-up as one take or claim verified spend within $0.10.

Both responses support the explicit 0–100 scale, but neither is an identical-audio comparison or proof of calibration. Application code, historical scores and scoring weights were unchanged in this follow-up.

Owner check: open [the isolated preview](https://delivery-production-0577.up.railway.app/play), sign in, record a take, listen, judge, replay and retry; confirm the saved take appears in history and plays after reloading. Report the actual device/browser and any unclear state. There is no demonstrated shared recording/storage defect from these guest checks; signed-in persistence is still a concrete shared-flow validation gap. Human playtesting is a launch limitation, not by itself a prohibition on developing Say It Back. The combined request allowance is exhausted; further paid checks need their own bounded authorization.
