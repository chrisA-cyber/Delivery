...
Current execution — September 6, 2026: guest-to-account return handling and community rounds extend the shipped friend-round system in PR #4. Hosts create Classic or Say It Back assignments, share a code/QR, privately review consented submissions, choose up to twelve showcase entries, operate a separate broadcast display, open/close audience voting, and start a linked next round. The display follows that next round; selected performances remain replayable during the existing retention window. Railway is the playable backend-connected application. See [COMMUNITY_ROUNDS_HANDOFF.md](COMMUNITY_ROUNDS_HANDOFF.md) for revisions, verification, limits, and the remaining owner session. No new paid calls: $1.675 of the original $5 cumulative allowance. Real provider login/account return, physical phone recording, and subjective broadcast audio acceptance remain unverified; authenticated claim invariants are covered by SQL and application tests. Switch, tournaments, public matchmaking, social feeds, and native streaming integration remain out of scope.

Working draft, updated September 6, 2026. Classic, Say It Back, private asynchronous groups, and bounded host-controlled community rounds are implemented. Earlier gates below are historical planning context. The owner's current authorization covers implementation, additive migrations, Git publication, merging after checks, and deployment within existing resources.
