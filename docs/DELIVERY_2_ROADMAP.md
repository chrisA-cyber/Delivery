...
Current execution — September 6, 2026: the recording-editor follow-up, seven genuinely longer Say It Back scenes, and asynchronous friend rounds through rematch are shipped and merged in PR #3. Railway serves the verified application `ed38bdb`; main release merge `c2f4cfe` also deployed successfully to Netlify, whose backend remains unconfigured. Focused live verification passed 49 HTTP requests plus cloud-browser create/join/replay/rematch. GitHub OAuth already works according to the owner; invited friends can participate through the scoped guest path without GitHub. The authoritative milestone record is [FRIEND_ROUNDS_HANDOFF.md](FRIEND_ROUNDS_HANDOFF.md). Paid evaluation remains $1.675 of the original $5 cumulative allowance, with no new paid provider tests in this milestone. The owner authorized additive migrations, merging completed work, and deployment. Historical phase gates and never-merge instructions below do not override that authorization; streamer expansion, Switch, and payments remain outside this milestone.

Working draft, updated September 6, 2026. Classic and Say It Back are established; the current milestone adds private asynchronous groups. Streamer participation and Switch remain later proposals. Camera-free participation remains central. This document records proposals for planning; it does not authorize implementation or deployment by itself. Update it as product decisions and implementation evidence arrive.
...
Subsequent environment setup created the approved $0/month `delivery-classic-test` Supabase project and applied/verified all 17 existing migrations plus seed. The isolated Railway preview is now running with native Redis and owner-configured private credentials. Three real OpenAI judgments of stock synthetic voices verified literal accuracy (100 for the correct line, 70 for changed words), guest quota progression and idempotent replay. Positive feedback with very low performance numbers exposed possible scale ambiguity; rubric v1.2 clarifies the existing 0–100 contract without rescaling history or changing weights. One authorized follow-up confirmed the deployed v1.2 rubric: 70 overall, 100 accuracy, 3.827 seconds, with specific quiet-direction coaching. The original WAV expired; regenerated audio differs, so this is not an identical-audio before/after comparison or proof of calibration. Feedback stability and signed-in save/playback remain unverified. Resume from [CLASSIC_RAILWAY_HANDOFF.md](CLASSIC_RAILWAY_HANDOFF.md). Human playtesting remains a launch limitation and does not by itself prohibit next-mode development; assess demonstrated shared-flow failures separately.
...
Delivery is a social performance game with several comfortable ways to participate: perform with voice or optional camera, choose directions, guess, react, or judge with friends. Its initial games are a substantially improved Classic and Say It Back, with Switch following group and streamer expansion. Players receive understandable feedback, improve their skills, invite groups, and join streamer-led sessions. A completed performance can become a finished, shareable clip that invites someone else to play the same challenge.
...
| --- | --- | --- |
| Quick play | Enjoy a challenge immediately; practice and improve | Reworked Classic and Say It Back first; Switch follows; Daily across supported modes; Speed/Rush and Gauntlet later |
...
Switch adds a sequence of changing directions to one continuous take. Classic is the simplest entry point; Say It Back is the next core mode, and Switch follows later. Both use the same visual language, media controls, result conventions, and invitation system. Implement them in small sequential assignments so the first polished Classic flow establishes reusable pieces for Switch.
...
Phase 3 is the first intended closed beta across solo, group, and streamer use. An initial public release can follow once quality and launch gates pass. It includes useful group and streamer participation; multi-person live media and advanced creator tooling follow later. Phases 4–6 are a proposed order, not a commitment to build every item.
...
1. Preserve Classic’s earlier connected-account evidence, private history/playback, duplicate-safe retries, and working GitHub OAuth. Email signup/recovery still requires verified SMTP; it does not block guest friend rounds.
2. Recording follow-up is implemented: precise displayed line windows and media-clock stopping, smaller capture blocks, measured saved/full-take waveforms, isolated retakes, immediate assembled preview, and immutable round-editor assignments. No arbitrary timing offset, speech stretching, scoring-weight change, or backend speed claim was introduced.
3. Seven new scenes add 10.60–17.55 seconds of actual footage and 2–5 recording lines each, including four Clean scenes. Seventeen immutable versions now ship in source. Existing versions and the shared 20-second recording limit are preserved; source/reuse limitations remain documented in [SAY_IT_BACK_MEDIA.md](SAY_IT_BACK_MEDIA.md).
4. Friend rounds are implemented at `/rounds`: one immutable Classic or Say It Back assignment, up to 12 people, guest display names, explicit private submission/replacement, deadline or host reveal, authorized replay, compatible score comparisons, a separate funniest vote, invitation revocation, and rematch. Optional unscored takes are visibly unreviewed and remain inside the invited group; known rejected/review-held recordings are blocked. Replay lasts seven days after closure, bounded to fourteen days from creation.
5. The current milestone is integrated, merged, and deployed. Complete only the brief desktop/phone microphone acceptance session in [the milestone handoff](FRIEND_ROUNDS_HANDOFF.md), then prioritize demonstrated issues from a small friend group. Do not repeat the completed paid batch or broad audits.
6. After publication, complete one brief desktop/phone owner session and a small real-friend round. Fix observed capture, replay, invitation, or comprehension problems. Do not turn this into another broad audit, paid evaluation batch, or expansion into live rooms, streamer broadcasting, Switch, or billing.
