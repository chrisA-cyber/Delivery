...
**Performance videos — shipped September 7, 2026**

Classic, five-cue Switch, and eligible Say It Back scenes now generate private, finished 1080×1920 H.264/AAC videos with actual-file preview, download, supported native file sharing, and exact public assignment invitations. Guest/unscored takes export without judging or invented scores. Tested application `f62b84260d513f82b35a59800d0b6b04e2144105` is live on Railway; all four representative live files passed duration, audio, cue/caption synchronization, framing, privacy, retry, and assignment checks. All 69 live HTTP checks passed; worker lease recovery, stale-publication rejection, and verified fixture cleanup were exercised. Application gates passed with 435 tests and three skipped provider opt-ins; database gates passed 713 assertions and 12 upgrade checks. Existing source/content restrictions, retention, deletion, and Netlify redirects remain; no new recurring service or AI spend was added, and the bound remains $2.675/$5. See [VIDEO_EXPORTS_HANDOFF.md](VIDEO_EXPORTS_HANDOFF.md) for evidence and operational limits. Next is a brief physical-phone preview/download/share check, audible OBS acceptance, and a small friend session; cloud HTTP/component/browser evidence does not replace those checks.

**Roast Off — deployed with actual media verification September 7, 2026**

One moderated public live stage and signed-in private hosts now use LiveKit Cloud with receive-only spectators, optional cameras, explicit device/roast consent, challenger queue/acceptance, server-owned alternating turns, audience chat/reactions/voting, winner-stays rotation and host moderation. Redis serializes state and media permissions; Supabase retains only expiring results/reports. Initial cap is two rooms, 28 connections per room, 12 queued challengers and one hour per session, under the existing-resource/LiveKit Build allowance. No recording, AI judging, tournaments, payments or live exports. The actual LiveKit run passed 34 checks with five native clients, including a complete battle/vote, next challenger, disconnect recovery, publishing revocation and resource/account cleanup. Median connection was 104 ms and native decoded-audio delay 149 ms in that run. Application gates passed 67 focused tests plus TypeScript/lint/build; database checks passed 75 assertions. Desktop entry pages were inspected; independent browser/device and phone acceptance remain explicitly unverified. See [ROAST_OFF_HANDOFF.md](ROAST_OFF_HANDOFF.md) for release evidence, cost assumptions and the short physical-device acceptance session.
