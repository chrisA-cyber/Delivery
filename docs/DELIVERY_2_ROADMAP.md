...
5. Finish the current integration: apply the additive migration/import to the intended Delivery resources, run required gates and focused multi-session service/browser checks, merge, and verify that Railway receives the final code. Record Netlify’s resulting status separately. Publication and live evidence are pending in [the milestone handoff](FRIEND_ROUNDS_HANDOFF.md), not assumed complete here.
