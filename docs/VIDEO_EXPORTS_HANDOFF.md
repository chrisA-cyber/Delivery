...
The existing microphone is captured once. Its unprocessed PCM WAV remains the playback, judging and dub-mix source. MediaRecorder observes the same microphone with the camera; its audio is never played or mixed again. Capture starts share the source-graph clock, avoiding main-thread PCM callback delay in the camera offset. Finishing waits for the encoded camera tail before publishing the paired take. Cancellation retains the previous pair; device loss, backgrounding and limits stop capture and release devices. Browser encoding prefers WebM/VP8+Opus with MP4 fallback.

Say It Back keeps camera intervals on the same scene timeline as accepted audio. Full-scene assembly, per-line replacement and saved-take retakes retain untouched intervals and source offsets. Replay follows the existing audio or scene element directly. The clip editor adds Camera/Avatar presentation, direct frame movement/crop/resize and keyboard/slider controls for zoom, crop, placement, size and mirroring. Saved settings reopen with the original performance. Scene layouts preserve the reference image's aspect ratio; trimmed voice, camera, captions and cues share the source clock.

Renderer `delivery-vertical-v5-camera` uses the existing serial Railway FFmpeg worker and 1080×1920 H264/AAC output. Camera metadata is autorotated before cropping; only the original WAV and established scene backing mix are mapped to audio. Camera takes can also render with their avatar. Existing download, native-share fallback and private export recovery remain.

Migrations `20260907220147_performance_camera_media.sql` and `20260907235901_camera_retakes_cleanup.sql` are applied. Retake cleanup deduplicates repeated source paths before scheduling deletion. Camera companions use the private `delivery-camera` bucket, a 32 MB aggregate limit, a 46-second media bound, service-only manifests and the existing guest/account/source authorization. Uploads reserve immutable paths before writing; matching retries complete the same reservation. Source deletion, account deletion, guest adoption, expiry and pending-upload cleanup reuse source lifecycle hooks and the existing worker. Tombstones persist for 24 hours to remove late uploads. Round and broadcast camera routes run only after their existing audio/consent/host authorization; recording does not grant publishing consent. Friend-sharing copy explicitly includes recorded camera footage.

Verification: production build, scoped lint/type checks and 133 focused recording/editor/dub/source/playback tests. Real Chrome MediaRecorder files were captured from simulated canvas/audio inputs in landscape and portrait. Actual MP4 renders cover all three modes, a Say It Back line replacement, a 90-degree metadata fixture, camera-to-avatar fallback, crop and trim. Neighboring retake audio is sample-identical; finished audio gain is within 0.1% of the established original/mix and checked camera patches average about 2–3 pixel levels of compression difference. See [render results](evidence/camera/render-results.json), [media comparisons](evidence/camera/media-agreement.json), and [contact sheet](evidence/camera/render-contact-sheet.webp). Synthetic input clocks are not physical-device lip-sync acceptance. Phone cameras, native Safari capture and native share sheets require device acceptance; the MP4 format/rotation fixture is converted media, not a Safari recording.

Railway still pins `source.commitSha`; update the existing Delivery service to the pushed main revision and explicitly deploy. Feature deployment `076373b8-d168-4a9a-97d2-3cc7d375dd2e` succeeded from `b801eb9da51040f9c07654c6800b0e060966c6e9`. All three live guest upload/retry/private replay/edit/reopen/render/download flows passed, including five camera intervals in the assembled Say It Back retake and Classic camera-to-avatar export. Original WAV and camera bytes remained identical, other guests received 404, and all downloaded MP4s fully decoded. See [live checks](evidence/camera/live-checks.json). Live cleanup caught and fixed a repeated-segment tombstone collision; owner deletion now removes all three fixture takes, and the existing worker removed all four camera objects. A transactional regression covers repeated source paths. The final source also adds permission-pending Avatar cancellation and corrects old camera-free copy; the release report identifies that deployed revision. Clipping is untouched.
